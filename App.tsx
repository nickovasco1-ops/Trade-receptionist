...
// --- Header ---
const Header = ({ currentView, onViewChange, onWaitlist }: { currentView: View, onViewChange: (view: View) => void, onWaitlist: () => void }) => {
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-10">
        <div className="flex justify-between h-20 items-center">
          ...
          <div className="hidden xl:flex items-center space-x-4 pl-4">
            ...
            <Button variant="primary" size="sm" onClick={onWaitlist}>Join Waitlist</Button>
          </div>
        ...
        <div className="xl:hidden bg-white border-b border-slate-100 absolute w-full px-6 py-8 shadow-2xl animate-in slide-in-from-top-5">
          <div className="flex flex-col space-y-6">
             ...
             <div className="flex flex-col gap-4">
                ...
                <Button variant="primary" fullWidth onClick={onWaitlist}>Join Waitlist</Button>
             </div>
          </div>
        </div>
      )}
        ...
        <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-10 relative z-10">
          <div className="grid lg:grid-cols-2 gap-16 lg:gap-12 items-center">
            <div className="text-center lg:text-left pointer-events-none md:pointer-events-auto">
              ...
              <div className="flex flex-col items-center lg:items-start gap-3 pointer-events-auto">
                <div className="flex flex-col sm:flex-row gap-5 justify-center lg:justify-start w-full sm:w-auto">
                  <Button variant="primary" size="lg" className="w-full sm:w-auto text-lg px-10 h-16" onClick={onWaitlist}>
                    Join Waitlist
                  </Button>
                ...
                <p className="text-sm text-slate-500 font-bold tracking-tight">Be the first to use the platform for free.</p>
              </div>

              <div className="flex flex-wrap items-center justify-center lg:justify-start gap-6 md:gap-8 text-sm md:text-base text-slate-500 font-semibold tracking-wide mt-12">
  ...
  return (
    <Section id="pricing" bg="white">
      <div className="text-center mb-16">
        ...
        <p className="text-xl text-slate-600 mb-10">No contracts. Cancel anytime. Early access starting soon.</p>
        <div className="inline-flex bg-slate-100 p-1.5 rounded-full relative">
          <button onClick={() => setBilling('monthly')} className={`px-8 py-3 rounded-full text-sm font-bold transition-all ${billing === 'monthly' ? 'bg-white text-tradeBlue-900 shadow-sm' : 'text-slate-500'}`}>Monthly</button>
          <button onClick={() => setBilling('yearly')} className={`px-8 py-3 rounded-full text-sm font-bold transition-all ${billing === 'yearly' ? 'bg-white text-tradeBlue-900 shadow-sm' : 'text-slate-500'}`}>Yearly <span className="text-brand-600 text-[10px] ml-1 font-extrabold uppercase tracking-wide">-20%</span></button>
        </div>
      </div>
      <div className="flex md:grid md:grid-cols-3 gap-8 overflow-x-auto snap-x snap-mandatory pb-8 md:pb-0 px-4 md:px-0 -mx-4 md:mx-0 no-scrollbar pt-6">
        {plans.map((plan, i) => (
          <div key={i} className={`snap-center flex-shrink-0 w-[85vw] md:w-auto relative flex flex-col p-10 rounded-3xl border transition-transform duration-300 hover:-translate-y-2 ${plan.isPopular ? 'border-brand-500 bg-white ring-8 ring-brand-500/5 shadow-2xl' : 'border-slate-200 bg-white hover:shadow-xl'}`}>
             {plan.isPopular && <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-brand-600 text-white text-[10px] uppercase font-bold tracking-widest px-4 py-1.5 rounded-full shadow-lg">Most Popular</div>}
             ...
             <ul className="space-y-5 mb-10 flex-1">
                {plan.features.map((feat, k) => (
                  <li key={k} className="flex items-start gap-3 text-sm text-slate-600 font-medium"><CheckCircle2 className="w-5 h-5 text-brand-500 flex-shrink-0" />{feat}</li>
                ))}
             </ul>
             <Button variant={plan.isPopular ? 'primary' : 'outline'} fullWidth onClick={onWaitlist}>{plan.buttonText}</Button>
          </div>
        ))}
      </div>
    </Section>
...
const FAQ = () => {
  ...
  const faqs: FAQItem[] = [
    { question: "Do I need to change my phone number?", answer: "No. You simply set up call forwarding on your existing mobile or landline." },
    { question: "How does the AI know my prices?", answer: "During onboarding, you provide a simple knowledge base. The AI uses this to answer questions intelligently." },
    { question: "What if it's an emergency?", answer: "You can set rules for urgent keywords. The AI can instantly transfer these calls to your mobile." }
  ];
  return (
    <Section bg="gray" id="faq">
      <div className="text-center mb-16"><h2 className="text-3xl md:text-4xl font-extrabold text-tradeBlue-900 mb-6 tracking-tight">Frequently Asked Questions</h2></div>
      <div className="max-w-3xl mx-auto space-y-4">
        {faqs.map((faq, i) => (
          <div key={i} className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden transition-all duration-300">
            <button onClick={() => setOpenIndex(openIndex === i ? null : i)} className="w-full flex items-center justify-between p-6 md:p-8 text-left focus:outline-none hover:bg-slate-50 transition-colors">
                <span className="font-bold text-lg text-tradeBlue-900 pr-8">{faq.question}</span>
                {openIndex === i ? <ChevronUp className="w-6 h-6 text-brand-500 flex-shrink-0" /> : <ChevronDown className="w-6 h-6 text-slate-400 flex-shrink-0" />}
            </button>
            {openIndex === i && <div className="px-6 md:px-8 pb-8 text-slate-600 leading-relaxed text-base border-t border-slate-100 pt-6">{faq.answer}</div>}
          </div>
        ))}
      </div>
    </Section>
        ...
        <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-10">
            <div className="grid md:grid-cols-4 gap-12 mb-16">
                <div className="col-span-1 md:col-span-2">
                    <div className="flex items-center gap-2 mb-8"><div className="w-[120px] md:w-[140px]"><Logo className="w-full" /></div></div>
                    <p className="max-w-xs text-base leading-relaxed mb-8 text-slate-500 font-medium">The UK's #1 AI receptionist app for tradespeople. Never miss a call again.</p>
                </div>
                <div>
                    ...
                    <ul className="space-y-4 text-sm font-medium text-slate-500">
                        ...
                        <li><button onClick={onWaitlist} className="hover:text-tradeBlue-900 transition-colors text-left">Join Waitlist</button></li>
                    </ul>
                </div>
            </div>
            <div className="border-t border-slate-100 pt-10 flex flex-col md:flex-row justify-between items-center gap-6">
                <p className="text-sm font-medium text-slate-400">&copy; 2024 Trade Receptionist Ltd. All rights reserved.</p>
                <div className="flex gap-8 items-center text-slate-400">
                   <a href="https://instagram.com/tradereceptionist" target="_blank" rel="noopener noreferrer" className="hover:text-brand-600 transition-colors"><Instagram className="w-6 h-6" /></a>
                   <a href="https://tiktok.com/@tradereceptionist" target="_blank" rel="noopener noreferrer" className="hover:text-brand-600 transition-colors"><TikTokIcon className="w-6 h-6" /></a>
                   <a href="https://www.facebook.com/share/16QddwsMk8/" target="_blank" rel="noopener noreferrer" className="hover:text-brand-600 transition-colors"><Facebook className="w-6 h-6" /></a>
                </div>
            </div>
        </div>
    </footer>
    ...
    <Section bg="white" className="border-t border-slate-100">
        <div className="bg-tradeBlue-900 rounded-[3rem] p-10 md:p-20 text-center text-white relative overflow-hidden shadow-2xl">
            ...
            <div className="relative z-10 max-w-3xl mx-auto">
                ...
                <p className="text-xl text-slate-300 mb-12 leading-relaxed">Join 500+ UK trades businesses saving 10+ hours a week. Be the first to use the platform for free.</p>
                <div className="flex flex-col items-center gap-4">
                  <div className="flex flex-col sm:flex-row justify-center gap-6 w-full sm:w-auto">
                      <Button variant="primary" size="lg" className="h-16 px-10 text-lg" onClick={onWaitlist}>Join Waitlist</Button>
                  ...
                  <p className="text-sm text-slate-400 font-bold">Be the first to use the platform for free.</p>
                </div>
            </div>
        </div>
    </Section>
