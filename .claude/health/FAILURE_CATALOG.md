...
| **Evidence** | `.github/workflows/notification-health.yml` has been **failing daily for 29+ days** (Sentry `API-2`, Twilio 21211) with no action taken · `server/src/lib/emergency.test.ts` was written against `bun:test` in a repo with no Bun — **44 tests that had never once executed** until ported to `node:test` on 2026-08-30 · the Playwright suite had **20 hard failures** and was excluded from CI until `aa4b6ac` |
