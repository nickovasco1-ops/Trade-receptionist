...
// CI runs each `playwright test` invocation in isolation, so the smoke run must
// start its own web + API servers (the accessibility run's servers are already
// torn down). Mirrors the webServer setup in playwright.config.ts.
const e2eEnv = {
  ...process.env,
