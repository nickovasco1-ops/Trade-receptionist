...
| Notion | `server/src/services/notion.ts`, `server/src/services/notion-sync.ts`, `server/src/services/revenue-tracker.ts` | **Optional internal ops dashboard** — call log, subscriber signups, incident audit trail, reconciled plan usage and Stripe-backed revenue tracking. The subscriber sync creates missing managed usage columns and emails one operational alert at 80%/100% per allowance period; the revenue sync updates the existing inline-table page every two hours. Lazy-inits and silently no-ops if `NOTION_API_KEY` is unset — never blocks the call/webhook pipeline. | Fire-and-forget by design; never let a Notion write or usage alert become load-bearing for the call flow. Supabase call rows, not the append-only Notion Call Log, are usage truth. Stripe subscriptions, not Notion or `clients.plan`, are revenue truth. |
...
| `POST /admin/sync-notion` | Reconciles Subscribers and Leads from Supabase and the inline-table Revenue Tracker from Stripe. Triggered every two hours by `.github/workflows/notion-sync.yml`; one failed result fails the workflow, while an unset optional target is reported as skipped. |
...
| `NOTION_API_KEY` + database IDs; optional `NOTION_REVENUE_PAGE_ID` override | Fully optional | No-ops when absent — never blocks the call pipeline. Revenue Tracker defaults to the existing live page id and also requires `STRIPE_SECRET_KEY`. |
