...
| Notion integration access silently detaches | The Subscribers and Call Log databases stopped being shared with the **trade receptionist** integration around 2026-06-15. Every `notion.ts` call write and every `syncSubscribers()` row has failed since, answered with `Could not find database with ID …`. `notion.ts` is fire-and-forget so nothing broke loudly; the sync workflow emailed a bare `failed: 5` every two hours for **25 consecutive runs** because the cause was only in Sentry. Env vars were correct throughout. | Access is Notion-side config, not code — connect the integration to the **Trade Receptionist HQ** page and every database under it inherits (`docs/NOTION.md`). `syncAll()` now probes each database once up front and returns the provider's message in `SyncResult.reason`, which `notion-sync.yml` prints; a failure alert that carries no reason is the bug, not the failure. |
