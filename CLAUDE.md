...
| Notion | `server/src/services/notion.ts`, `server/src/services/notion-sync.ts` | **Optional internal ops dashboard** — call log, subscriber signups, incident audit trail and reconciled plan usage. The subscriber sync creates missing managed usage columns and emails one operational alert at 80%/100% per allowance period. Lazy-inits and silently no-ops if `NOTION_API_KEY` is unset — never blocks the call/webhook pipeline. | Fire-and-forget by design; never let a Notion write or usage alert become load-bearing for the call flow. Supabase call rows, not the append-only Notion Call Log, are usage truth. |
