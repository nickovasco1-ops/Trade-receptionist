...
| `POST /admin/check-calendars` | Probes every active tenant's diary and reports which credentials the provider has rejected, flagging `calendar_status='needs_reconnect'` as it goes. **Intended daily, before the working day** (`.github/workflows/calendar-health-cron.yml`, 06:30 UTC) — it exists because Google expires refresh tokens after 7 days while the OAuth project is in Testing, and a diary that died overnight is otherwise first noticed by a caller asking for an appointment, which the LLM talks around silently. Emails `CALENDAR_ALERT_EMAIL` (falling back to `INTEGRITY_ALERT_EMAIL`). The workflow fails on `needsReconnect > 0`, not merely on a non-2xx. |
...
| Google OAuth in Testing mode expires tokens after 7 days | Calendar scopes are "sensitive", so the project needs Google's verification review to reach Production. While the consent screen reads **Testing**: refresh tokens die after 7 days, users see an "unverified app" warning first, and the project is capped at 100 users for its entire lifetime, uncleanable. A tenant would connect their diary and silently lose it a week later. **Not verified from this environment — `developers.google.com` is egress-blocked, so the project's actual Publishing status was never confirmed.** | Check Google Cloud Console → OAuth consent screen → Publishing status before trusting any Google connection to last. Verification takes weeks of calendar time and commonly fails first pass, so start it early. The scope was narrowed from `auth/calendar` to `calendar.events` + `calendar.freebusy` — **freebusy must stay**, it is not covered by `calendar.events` and dropping it breaks every availability check while still compiling. |
