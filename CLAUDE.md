...
- Onboarding flow: `WelcomePage.tsx` → `OnboardingPage.tsx` → `DashboardPage.tsx`. The wizard's seven steps are receptionist → business → services → hours → **diary** → contact → ready. The diary step renders `components/dashboard/DiaryConnect.tsx`, which is shared with Settings so there is exactly one connect flow.
...
- Schema lives in `supabase/migrations/*.sql` (19 migrations as of 2026-09-22). Treat migrations as append-only; never edit a committed migration — write a new one.
...
| Stripe | `components/StripeCheckoutModal.tsx` (static Payment Links per plan, `src/lib/plans.ts`) + `server/src/routes/webhooks/stripe.ts` | Subscriptions / billing. `checkout.session.completed` runs the **entire tenant provisioning pipeline** (Retell agent + Twilio number + Supabase auth user + welcome email) — this is the most consequential webhook in the codebase. | Webhook signature is verified by hand (HMAC-SHA256 of `timestamp.rawBody`, `timingSafeEqual`) in `routes/webhooks/stripe-signature.ts` — **not** the Stripe SDK. It **fails closed** (2026-08-30): a missing secret, a missing `stripe-signature` header, or a timestamp outside the 300s window all reject. `STRIPE_PRODUCT_TO_PLAN` in `server/src/lib/stripe-plans.ts` hardcodes live+test Stripe product IDs — a new/changed Stripe product needs this shared map updated or provisioning and revenue sync both fail to classify it correctly. |
...
| Calendars (Google / Microsoft / Apple) | `server/src/services/calendar.ts` (availability), `calendar-providers.ts` (adapters + connection resolution), `calendar-connect.ts` (OAuth + CalDAV), `lib/caldav-xml.ts` (pure iCal/WebDAV parsing), `routes/auth` | Per-tenant connection to whichever diary the trade actually uses; the agent checks free/busy live mid-call via `retell-tools` and books straight in. | OAuth `state` is HMAC-signed **and now expires after 15 minutes** (`GOOGLE_OAUTH_STATE_SECRET`, falling back to `SUPABASE_SERVICE_ROLE_KEY`) — it previously carried an `issuedAt` that nothing checked, so a captured callback URL worked forever. Token exchange is server-side only. A provider rejecting a stored credential raises `CalendarAuthError`, which sets `calendar_status='needs_reconnect'` and logs at error level; a plain outage does not. The message carries the provider's own code via `authRejectionDetail()` (e.g. `HTTP 400 invalid_grant: Token has been expired or revoked.`) — `invalid_grant` means the customer's access was withdrawn, `invalid_client`/`unauthorized_client` means **our** client ID/secret is wrong and every tenant on that provider is down. Until 2026-09-26 the provider's answer was discarded and the only way to learn why a diary died was to ask the customer. |
| Notion | `server/src/services/notion.ts`, `server/src/services/notion-sync.ts`, `server/src/services/notion-operations.ts`, `server/src/services/revenue-tracker.ts` | **Optional internal ops dashboard** — call log, subscriber signups, incident audit trail, reconciled customer attention and plan usage, a working lead queue, objective onboarding stages, Stripe-backed revenue, and the Owner's Daily Dashboard. The subscriber sync creates missing managed columns and emails one operational alert at 80%/100% per allowance period; all reconciled surfaces refresh every two hours. Lazy-inits and silently no-ops if `NOTION_API_KEY` is unset — never blocks the call/webhook pipeline. | Fire-and-forget by design; never let a Notion write or usage alert become load-bearing for the call flow. Supabase call rows, not the append-only Notion Call Log, are usage truth. Stripe subscriptions, not Notion or `clients.plan`, are revenue truth. The owner dashboard must show `Unavailable — check sync`, never zero, when a source failed or was skipped. |
...
- Stripe lifecycle events (`invoice.payment_succeeded/failed`, `customer.subscription.deleted`) update `clients.subscription_status`/`payment_status`/`is_active`/`current_period_start`/`current_period_end`, matched in priority order: `stripe_subscription_id` → `stripe_customer_id` → `owner_email`. The period bounds drive exact monthly call allowances in Notion; migration 020 falls back to the stored end/creation date until the next invoice supplies both bounds.
...
| `POST /admin/check-calendars` | Probes every active tenant's diary and reports which credentials the provider has rejected, flagging `calendar_status='needs_reconnect'` as it goes. **Intended daily, before the working day** (`.github/workflows/calendar-health-cron.yml`, 06:30 UTC) — it exists because a credential can die at any time — revoked access, a changed password, a rotated app-specific password — and a diary that died overnight is otherwise first noticed by a caller asking for an appointment, which the LLM talks around silently. (It was originally justified by Google's 7-day Testing-mode expiry, which turned out never to have applied; the sweep is still right, for the broader reason.) Emails `CALENDAR_ALERT_EMAIL` (falling back to `INTEGRITY_ALERT_EMAIL`). The workflow fails on `needsReconnect > 0`, not merely on a non-2xx. |
...
| `POST /admin/sync-notion` | Reconciles Subscribers, Live Leads and Onboarding Tracker from Supabase, the inline-table Revenue Tracker from Stripe, then the Owner's Daily Dashboard from those results. Triggered every two hours by `.github/workflows/notion-sync.yml`; one failed result fails the workflow. The live leads/onboarding/dashboard ids are code defaults and their env vars are replacement-page overrides. |
...
Seven tables (`supabase/migrations/002_revised_schema.sql` onward — `001_initial_schema.sql` created a different schema that 002 drops entirely, so treat 001 as dead history, not a reference):

| Table | Purpose | Notable columns added later |
|---|---|---|
| `clients` | One row per tenant | `own_number` (003), `onboarding_complete` (005), Stripe lifecycle columns — `stripe_customer_id`, `stripe_subscription_id`, `subscription_status`, `payment_status`, `current_period_end` (011), lowercased-email trigger (012), `plan` check widened to include `'business'` (018), multi-provider calendar columns — `calendar_provider`, `calendar_id`, `calendar_credentials`, `calendar_status`, `calendar_last_error`, `calendar_connected_at`, `calendar_checked_at` (019; the `google_*` pair is kept and written in step), exact allowance boundary `current_period_start` (020) |
...
| `usage_alerts` | Service-role-only audit of plan-usage warnings | unique per `client_id` + `period_start` + 80/100 threshold (020); no browser RLS policy |
...
| `STRIPE_SECRET_KEY` | Warn-only locally, **required staging/prod** | Server boots without it but checkout/billing silently breaks. |
| `STRIPE_WEBHOOK_SECRET` | Warn-only at boot, **load-bearing everywhere** | Not a boot check, but since 2026-08-30 the webhook **fails closed**: unset ⇒ every Stripe event is rejected (`stripe.webhook.secret_missing`, error-level) and no signup provisions. Must be set in Railway, CI (`.github/workflows/ci.yml` writes it into `.env.test`) and local `.env.test`. |
...
| `NOTION_API_KEY` + append-only/subscriber database IDs; optional `NOTION_LEADS_DB_ID`, `NOTION_ONBOARDING_DB_ID`, `NOTION_REVENUE_PAGE_ID`, `NOTION_OWNER_DASHBOARD_PAGE_ID` overrides | Fully optional | No-ops when the key is absent — never blocks the call pipeline. Live Leads, Onboarding Tracker, Revenue Tracker and Owner's Daily Dashboard default to their existing live page ids; Revenue Tracker also requires `STRIPE_SECRET_KEY`. |
...
| `ALERT_EMAIL`, `INTEGRITY_ALERT_EMAIL` | Optional | `ALERT_EMAIL` receives operational events including plan usage at 80%/100%, falling back to `INTEGRITY_ALERT_EMAIL`. Unset = the event is logged but nobody is emailed. |
...
| Retell list endpoints deprecate on a schedule, and the replacements change shape | Sept 2026: `POST /v2/list-calls` and `GET /list-phone-numbers` deprecated. **The replacements do not error — they answer differently.** `/v3/list-calls` returns `{ items, has_more, pagination_key }` instead of a bare array, renames the filter from `agent_id: [id]` to `agent: [{ agent_id }]`, and **drops `transcript` from the list item entirely** (only `GET /get-call/{id}` still carries it). `/v2/list-phone-numbers` likewise returns `{ items }`. Any of those read as "no calls, no numbers" rather than a failure. `listCallsForAgent()` also returned `[]` on a non-2xx, so a dead endpoint made `/admin/sync-calls` report a clean zero. Second dated Retell deprecation to break this integration with no change on our side. | Both list calls now go through `retell-sdk`, which carries the current version — **never hand-write a Retell list path again**; the SDK moves with the API. `listCallsForAgent()` throws instead of returning `[]`, and `/admin/sync-calls` reports `failed` so a rejected run cannot look like an idle one. `providers.retell_list_contract` probes both endpoints live and asserts the `items` shape, and runs on every PR via the `retell-contract` CI job — the next deprecation should land as a red build, not as a backfill that silently recovers nothing. |
| Hand-written Retell paths that were never right | Moving the *list* endpoints to the SDK (v4.5) stopped at the list endpoints, on the stated reasoning that "agent and LLM lifecycle calls are stable". Auditing all 16 hand-written paths against the installed SDK found two that are not in it: `GET /get-call/{id}` (current: `/v2/get-call/{id}`) and `POST /publish-agent/{id}` (current: `/publish-agent-version/{id}`). Whether either legacy path still answers today was not verifiable from the dev environment (egress to `api.retellai.com` is blocked), and it does not matter much: the deprecation notice says unversioned paths move to `/v2` and "may be removed at any time". Neither fails loudly when withdrawn. `getCall()` returned `null` on any non-2xx, which `POST /calls/backfill/:id` — the documented single-call recovery path — reported as **404 "Call not found in Retell"** for calls that plainly existed. Worse, `applyTierToAgent()` fired four bare `fetch`es and read **none** of the responses, so a publish to a path that 404s returned `{ applied, version }` exactly as a success would: an agent could be reported as tiered and published while having no published version at all, which per §8.4b means the number routes to nothing and the phone stays silent. | Both now go through the SDK, and `applyTierToAgent()` throws instead of dropping responses. `providers.retell_path_contract` compares every `${BASE_URL}/…` literal in `server/src` against the paths the installed `retell-sdk` actually calls; it runs offline on every PR and was confirmed to FAIL on both original paths before being trusted. **A bare `await fetch(...)` whose response is never read is the bug** — not the wrong path, which is only how it showed up this time. |
...
| Advertised price ≠ what the Payment Link charged | `src/lib/plans.ts` advertised Starter £49 and Pro £89 while their live Payment Links still sold the original April prices, **£29 and £59** — every Starter and Pro signup was charged 40% less than the price it accepted, from the day the four-tier scheme shipped until 2026-09-10. `PRODUCT_TO_PLAN` even carried `// £49/mo` and `// £89/mo` comments against those products. Separately, the £249 Agency Payment Link sells `prod_UehtHB44FF2E1Y`, which **was not in `PRODUCT_TO_PLAN` at all**, and the new Business/Agency links carried no `metadata.plan` — so `planFromStripeSession()` fell through to `?? 'starter'` and **every £249 Agency signup was provisioned as Starter**: 50-call allowance and, per §8.4b, the cheapest voice with no Fast Tier. | **A Stripe Payment Link's price cannot be edited after creation** — correcting one means creating a new link and updating `LIVE_URLS`. Deploy the new URLs *before* deactivating the old links, or the site serves a dead link. Both `metadata.plan` and `PRODUCT_TO_PLAN` are now set for all four tiers; keep both, they are each other's backstop. When you change a `price` in `plans.ts`, the Stripe link is a separate artefact that does not follow — verify the amount in Stripe, never the comment. |
...
| `onboarding_complete` does not mean "activated" | The wizard's six steps are receptionist → business → services → hours → contact → ready. **There is no calendar step.** A tenant only ends up with `google_cal_id` by happening to sign in with Google, and `buildRetellTools()` gates the booking tools on that column — so a customer who signed up with email has an agent that can capture a lead but **can never book a job**. Both paying customers were in exactly that state (found 2026-08-31), with `onboarding_complete: true` and a dashboard that said nothing. See catalogue **C18**. | Do not read `onboarding_complete` as "has a working product" — it means "walked the wizard". Real activation is: calendar connected, carrier divert active, and at least one call received. The dashboard warns the tenant (`calendar-setup-banner`), and `activation.booking_is_possible` warns us. **The wizard now has a Diary step** (migration 019 / v4.7), so this is fixed for new signups — but `onboarding_complete` still only means "walked the wizard", and a tenant can legitimately skip the step. Real activation is still: diary connected, divert active, at least one call received. |
| Calendars are three providers, not one | A tradesperson's diary is Google, Microsoft or Apple iCloud — "iPhone calendar" and "Android calendar" are apps, not services. Until migration 019 only Google was supported, and "is a diary connected?" was spelled `!!client.google_cal_id` in ~30 places, so adding a provider was a thirty-site change that would silently have left half of them Google-only. **Apple cannot ever be one tap**: Apple publishes no OAuth for calendar data, so CalDAV with an app-specific password is the only route, and Cronofy/Nylas/OneCal all do the same underneath — treat any claim of "seamless iCloud connect" as that, wrapped. Microsoft, by contrast, needs no review at all: `Calendars.ReadWrite` is available to consumer Outlook.com/Hotmail accounts with no admin consent. | Resolve the connection through `calendarConnection()` in `services/calendar-providers.ts` — never read a provider column directly. Adding a provider means the union in `shared/types.ts` **and** a migration widening `clients_calendar_provider_check`, per the 018 lesson. Only free/busy, create and delete are provider-specific; availability (working hours, DST, slots) is shared and must stay that way. |
| The Google OAuth project is **In production**, not Testing — the 7-day expiry never applied | This file asserted for weeks that refresh tokens die after 7 days and that only listed test users can authorise. **Both are Testing-mode behaviours and the project has been In production.** Confirmed from the console 2026-09-24; it had been guessed at because `console.cloud.google.com` and `developers.google.com` are egress-blocked from the dev environment, and the guess drove real advice — "add Alex and Steve as test users" was the top recommended action for days and was never necessary. What is actually true while the app is in production with **unapproved** sensitive scopes: anyone may authorise, refresh tokens do not expire on a 7-day clock, every user sees an **"unverified app" interstitial** they must click through, and each grant consumes one of a **100-user lifetime cap that can never be reset** (3 consumed as of 2026-09-24). The cap and the warning both disappear once verification approves the scopes. | **Read the console, do not infer the mode from symptoms.** Note the cap counts any grant that requests an unapproved sensitive scope — including `LoginPage.tsx`'s Google sign-in, which requests calendar scopes for the zero-tap connection, so **every Google signup burns a lifetime slot whether or not that tenant ever uses a diary**. Dropping calendar scopes from the login button would spend slots only on tenants who actually connect one, at the cost of the zero-tap path; that is a trade-off, not a bug. The scope was narrowed from `auth/calendar` to `calendar.events` + `calendar.freebusy` — **freebusy must stay**, it is not covered by `calendar.events` and dropping it breaks every availability check while still compiling. |
| The web app has no `strictNullChecks` | Root `tsconfig.json` sets no `strict` flag at all, so the entire frontend is unchecked for null. `server/tsconfig.json` *is* strict, which makes the gap easy to miss when moving between them. Passing a `string \| null` into a `string` prop type-checks clean. | Guard nulls by hand in frontend code; `npm run typecheck` will not catch them. Turning `strict` on is desirable but is a large separate job — `App.tsx` alone is ~2,000 lines. |
| Test files were never type-checked | `server/tsconfig.json` excludes `src/**/*.test.ts` so tests are not emitted to `dist` — sensible — but that also meant they were never type-checked. `npm run build:api` reported clean while `emergency.test.ts` carried 27 type errors inherited from its `bun:test` origins, surfacing only as a runtime `TypeError`. Third member of this family, after the unquoted test glob and the bun-only suite. | `server/tsconfig.test.json` type-checks tests with `noEmit`, and `npm test` runs it first. **A check that silently does not apply is worse than no check, because it reads as green.** |
| `sync-calls` repaired nothing that already existed | `POST /admin/sync-calls` is the documented recovery path for a dropped Retell webhook, but it `continue`d the moment a `calls` row existed, and never touched `leads` at all. The event that actually goes missing is **`call_analyzed`** — the one carrying `call_summary` and `custom_analysis_data` — so the calls were all present while **20 of the first 30 had no analysis** and 17 of the first 20 leads held nothing but a phone number. The cron reported success every run. It also had its own cruder `deriveOutcome` (uppercase the summary's first word, look it up in a map), so any prose summary was filed as `enquiry`. | Extraction now lives in `server/src/lib/lead-extraction.ts` and is shared by the webhook and the backfill; `sync-calls` reconciles analysis + leads on existing rows and reports `repaired`. **A recovery path that only handles the absent row does not recover the common case** — the partial row is the common case. It refuses to overwrite a lead a human has annotated (`isLeadEmpty`). |
| `calls.outcome` was never updated after the call ended | `call_ended` usually arrives **before** Retell has analysed the call, so `deriveOutcome()` ran on an empty summary and fell through to `enquiry`; `call_analyzed` refreshed the transcript and the lead but never the outcome. Found 2026-09-25: **all 19 calls of the previous 30 days were stored as `enquiry`** while Retell's own analysis in `transcripts.raw_json` said no_answer ×9, lead_captured ×6, booked ×2 — including a call that had put a job in a customer's diary. Every outcome-based number on the dashboard was therefore meaningless. | `services/call-outcome.ts` now rewrites the outcome from `call_analyzed` **and** from `/admin/sync-calls` (so the six-hourly cron repairs history), and a booking row for the call outranks the analysis via `resolveOutcome()`. Emergency is judged on the analysis *before* a booking relabels the call, so escalation is unaffected. **A field written from an event that arrives before its data exists must be revisited when the data arrives.** |
| The booking tool trusted a misheard phone number over caller ID | `create-booking` used `args.caller_number ?? call.from_number`, so the LLM's transcription always won. On 2026-09-25 it dropped a digit from a UK mobile: Twilio rejected the confirmation SMS (21211) **and the short number was written into the tradesperson's diary event**, while the caller ID on the same request was correct. Nothing in the codebase validated a phone number at all. | `toE164()` / `chooseCallerNumber()` in `shared/phone.ts`: the heard number still wins when it is a real number (a caller may want texts on a different phone), otherwise caller ID is used and `retell_tools.caller_number_invalid` is logged with the digit count only. Owner mobiles are still stored unnormalised (e.g. `07…`) — Twilio has accepted that so far, but it is not validated anywhere. |
...
| Supabase migrations | Append-only, 20 files as of 2026-09-25 (see §8.9). Migration 001 created a schema that 002 immediately drops — don't treat 001 as representative of the live schema. | Never edit a committed migration. Add a new one. |
...
| `server` test glob silently skipped nested tests | `server/package.json`'s test script was `tsx --test src/**/*.test.ts` **unquoted**. `sh` has no `globstar`, so `**` collapsed to one level: only `src/lib/*.test.ts` ever ran. A new suite at `src/routes/webhooks/stripe-signature.test.ts` was invisible — it "passed" by never executing, the same shape as `emergency.test.ts` being written for `bun:test`. Now quoted (`"src/**/*.test.ts"`) so Node expands it and recurses. | After adding a test file, **check the reported test count went up**. A suite that runs zero tests exits 0. |
...
| Stripe webhook signature verification | Verified by hand (HMAC-SHA256 of `timestamp.rawBody`) in `routes/webhooks/stripe-signature.ts`, **not** via the Stripe SDK. **Now fails closed** (fixed 2026-08-30). Until then the guard read `secret && sigHeader && !verify(...)`, so a request that merely **omitted the `stripe-signature` header** skipped verification entirely — *setting the secret did not close it*. An unauthenticated caller could drive `provisionClient()` (buys a Twilio number, creates a Supabase auth user) or deactivate a real tenant with a forged `customer.subscription.deleted`. There was also no timestamp check, so any captured webhook replayed forever. | **`STRIPE_WEBHOOK_SECRET` is now load-bearing**: unset ⇒ every Stripe webhook is rejected and logged `error` as `stripe.webhook.secret_missing`, so signups stop. Confirm it is set in Railway *before* deploying. Keep verification in `stripe-signature.ts` — it is deliberately free of side-effecting imports so it stays unit-testable (`stripe-signature.test.ts`, 11 tests incl. the replay window). |
...
| Notion integration access silently detaches | The Subscribers and Call Log databases stopped being shared with the **trade receptionist** integration around 2026-06-15. Every `notion.ts` call write and every `syncSubscribers()` row failed, answered with `Could not find database with ID …`. `notion.ts` is fire-and-forget so nothing broke loudly; the sync workflow emailed a bare count for repeated runs because the cause was only in Sentry. Repaired 2026-09-25 by creating the integration in the workspace that actually owns **Trade Receptionist HQ**; a valid token from another workspace was the reason the connection never appeared in the page menu. | Access is Notion-side config, not code — integration and page must belong to the **same workspace**, then connect it to **Trade Receptionist HQ** so databases and dashboards beneath inherit (`docs/NOTION.md`). `syncAll()` probes each database once and returns the provider message in `reason`. Live Leads, Onboarding Tracker and Owner's Daily Dashboard have recorded live ids as defaults, so an unset override cannot silently remove them from the run. Subscriber plan usage is computed from Supabase, so a missing Notion call-log row never corrupts allowances; the sync may add known managed properties but must never remove user-owned columns. A downstream dashboard source failure must render `Unavailable — check sync`, not a plausible zero. |
| Revenue Tracker was a static page of zeros | The hand-built Notion page contained inline tables, not a database, and nothing updated it. Reading revenue from `clients.plan` would also be wrong because Stripe carries actual prices, legacy products and billing intervals. | `syncRevenueTracker()` reads all Stripe subscriptions and updates the existing tables during the two-hour Notion sync. Only `active` subscriptions count as MRR; trials and past-due subscriptions are separate. The first automated month is marked `*` because historic expansion/contraction cannot be reconstructed from a current snapshot. Unknown products, discounts, metered prices and mixed/non-GBP currency fail the sync rather than publishing a plausible but wrong number. Stripe product IDs live once in `lib/stripe-plans.ts`; provisioning and revenue must share that map. Month labels are deliberately fixed to three-letter English abbreviations and the parser accepts both `Sep` and Notion's already-written UK `Sept`; do not depend on `Intl`'s locale-specific abbreviation for an idempotency key. |
| `app.tradereceptionist.com` has never existed | Three server files hardcoded `https://app.tradereceptionist.com` as the dashboard base: the lead deep-link in **every owner SMS after every call**, the trial-reminder email, and the Stripe billing-portal return URL. `.env.example` also told Railway to send the calendar OAuth success redirect there. The subdomain has no DNS record and is not on the Vercel project — Vercel serves `tradereceptionist.com` (apex, verified) with `www.` 308-redirecting to it, and the dashboard is part of the same SPA at `/dashboard`. So every lead link a customer has ever been texted was dead, and a customer who connected a diary would have landed on nothing. Fixed 2026-09-22. | The dashboard base is `PUBLIC_APP_URL`, defaulting to `https://tradereceptionist.com`. **A hardcoded domain is not a configuration — check it resolves.** `docs/release-readiness.md` M5 flagged two of these sites and it was never actioned; the third (`retell.ts`) arrived later. |
| Google OAuth verification rejects any authorised domain you do not own | The consent screen's **Authorized domains** listed `tradereceptionist.com`, `pyeztxqgjdrguktydtez.supabase.co`, `require-volatile-directed.ngrok-free.dev` and `trade-receptionist-production.up.railway.app`. Google requires each one to be verified in Search Console under the support-email account, and three of the four are somebody else's parent domain — unverifiable by construction. The ngrok entry is leftover local-dev debris. `supabase.co` is there for `LoginPage.tsx`'s **Sign in with Google**, whose callback is `<ref>.supabase.co/auth/v1/callback` — and which still requests the broad `auth/calendar` scope the calendar flow was deliberately narrowed away from, widening the review for a button the tenants do not use (they arrive by magic link). | The callback belongs on the owned domain: `vercel.json` **already rewrites `/api/*` to Railway**, so `https://tradereceptionist.com/api/auth/google/callback` needs no CNAME, no new subdomain and no DNS change. `GOOGLE_REDIRECT_URI`/`MICROSOFT_REDIRECT_URI` are how you move it — and they were **dead code** until 2026-09-23: `redirectUriFor()` read `derived ?? override`, so with `PUBLIC_API_BASE_URL` set (it is, in production) the override was silently ignored while producing a URI that worked. Resolution now lives in `lib/oauth-redirect.ts`, pure and covered by 12 tests. **Authorized domains is a list of things you must own — treat any entry ending in someone else's domain as a rejection waiting to happen.** Google's two documented requirements for the remaining "is not registered to you" rejection, both of which must hold: (1) the Search Console property must be a **Domain property, DNS-verified** — a URL-prefix property does not count; (2) the account that verified it must be a **verified** owner (holding a real token) **and** an Owner or Editor of the Cloud project. **A delegated owner does not satisfy this** — Search Console's own docs define a delegated owner as one granted ownership "without the use of a verification token". There is a trap in between: while an account is a delegated owner, adding the Domain property returns *"Ownership auto verified — Verification methods: Unknown"* and Google never offers a TXT record, so the account cannot become verified. Break it by removing the delegated ownership from the *other* account first, then adding the property fresh — or, on a registrar Google integrates with (GoDaddy included), use the **"Domain name provider"** method, which authorises Google to write the TXT itself and reports the method by name instead of "Unknown". Resolved 2026-09-24 that way. The alternative fix, never needed in the end, is to add the already-verified account to the project in IAM as Owner. **Each rejected resubmission costs a multi-week review, so align every requirement before pressing "I have fixed the issues" — never submit to test a hypothesis.** |
| Every route serves an empty body to anything that does not run JavaScript | The SPA ships `<body><div id="root"></div><script/></body>`. A non-JS fetch of **any** route — the home page, `/privacy`, `/terms` — receives a blank document, which is indistinguishable from a dead site. Google's OAuth verification fetches all three and rejected the project with *"Your home page URL is unresponsive"*; the policy pages were worse, since their `<head>` carries no content either while the home page at least has meta tags and JSON-LD. Measured before the fix: **0 visible characters** at `/privacy`. | `npm run build` now runs `scripts/prerender-legal.mjs`, which renders the two legal pages with `renderToStaticMarkup` and writes `dist/{privacy,terms}/index.html`; Vercel checks the filesystem before the `/(.*)` rewrite, so those files are served without a config change, and React still hydrates over them. The home page gets a `<noscript>` body instead — `App.tsx` owns Lenis, IntersectionObserver and parallax and is not safely renderable on a server. **The prerender throws if the markup is under 500 characters or the root div is missing**, because a prerender that silently writes nothing reads as a clean build. `react-router-dom` is CommonJS and cannot be pulled through Vite's ESM SSR loader either externalised or bundled; the pages use only `<Link>`, so `scripts/prerender/router-stub.jsx` substitutes it. |
| Two OAuth clients ask Google for calendar access, and only one was narrowed | The Diary connect flow requests `calendar.events` + `calendar.freebusy`. **`LoginPage.tsx`'s "Sign in with Google" requested the full `auth/calendar` scope** — read and write over every calendar the person owns — until 2026-09-24. It is not dead code: `src/lib/calendar.ts` captures the returned `provider_refresh_token` and saves it as the zero-tap calendar connection, so the scope is load-bearing and could not simply be deleted. It went unnoticed because the v4.7 narrowing treated the server adapter as the only place that asks. Found while Google's OAuth verification was **live under review**, where "Request minimum scopes" is one of the seven categories a reviewer works through — an app whose login button demands full calendar access while its written justification says events+freebusy is a rejection waiting to happen. | One list, `GOOGLE_CALENDAR_SCOPES` in `shared/types.ts`, read by both sites. A unit test asserts the server constant equals it **and greps `LoginPage.tsx` for a hard-coded scope literal**; it was confirmed to FAIL against the pre-fix file before being trusted. **When you narrow a scope, grep for every other client that requests one** — the same lesson as the three signature verifiers. Note the Supabase Google provider is a separate OAuth client configured in the Supabase dashboard, so check there too if the consent screen ever shows more than these two scopes. |
...
*v4.9 · 2026-09-26 — Notion became an operating system rather than a collection of passive tables. The two-hour sync now builds an action-first customer health queue, writes every lead into a compatible Live Leads database, derives objective onboarding blockers, and rolls trusted Supabase/Stripe figures into an Owner's Daily Dashboard. Failed sources render as unavailable instead of silently becoming zero; the existing customer-facing call path remains fire-and-forget and independent. Server tests 169 → 175.*
*v4.9 · 2026-09-26 — a paying customer's Google diary died within a day of connecting, and the stored reason was a fixed sentence: the provider's own error had been thrown away, so the only way to find out why was to ring the customer. `authRejectionDetail()` now keeps the provider's code and first line (emails masked, capped) in `calendar_last_error`, Sentry and the alert email, and names an `invalid_client` outright as ours rather than the customer's. Also corrected two comments still claiming the 7-day Testing-mode expiry. Server tests 175 → 182.*
*v4.8 · 2026-09-25 — the first booking on a real tenant's diary exposed two data bugs. The outcome column had never been corrected after a call ended, so every call for a month read `enquiry` while Retell's analysis said otherwise — the dashboard could not show a tradesperson a single job their receptionist had won. And the booking tool trusted the LLM's transcription of a phone number over the caller ID, so a dropped digit failed the customer's confirmation text and put a wrong number in the diary. Outcomes are now rewritten when analysis arrives (live and in the six-hourly backfill, which repairs history), a booking outranks the analysis, and a number that cannot exist falls back to caller ID. Server tests 136 → 148.*
*v4.7 · 2026-09-22 — the headline feature had never been used. Of five tenants exactly one has ever connected a calendar, and that one is the founder's own test account; both paying customers keep their diary in Google **through** the iOS Calendar app, so "we support Google Calendar" was never the same claim as "our customers can connect one". Two structural causes: the wizard had no calendar step, so the only route in was a Settings page nobody opens, and "is a diary connected?" was spelled `!!client.google_cal_id` in ~30 places. Now three providers behind one `CalendarAdapter` — Google (scope narrowed to `calendar.events` + `calendar.freebusy`), Microsoft (new; Outlook/Hotmail/365, and it needs no review at all), Apple iCloud (new; CalDAV, and it cannot be one tap because Apple publishes no OAuth for calendars). A new Diary step asks "where do you keep your diary?" and, for "My iPhone", asks which email backs it — routing the common case to one tap and leaving app-specific passwords to genuine iCloud diaries. A rejected credential now flags `needs_reconnect` and logs at error level instead of the agent talking around a missing tool mid-call. Found in passing and fixed: the OAuth state had an unchecked `issuedAt`, so a captured callback URL worked forever; and `server/tsconfig.json` excluded test files from type-checking, hiding 27 errors in `emergency.test.ts`.*
*v4.6 · 2026-09-21 — Retell's weekly deprecation notice showed the legacy list traffic had stopped: 2 requests in seven days, the last at 20:36 UTC on 14 Sept, 59 minutes before the SDK fix merged. Nothing has used a legacy list endpoint since. But auditing the remaining hand-written paths against the installed SDK — which the v4.5 work did not do, having assumed the lifecycle calls were stable — found two that Retell does not serve at all: `/get-call/{id}` and `/publish-agent/{id}`. The first made call backfill answer "Call not found in Retell" for calls that existed. The second sat in `applyTierToAgent()`, which read none of its four responses, so an agent that never published was reported as successfully tiered — the failure mode §8.4b's `agent_never_published` check exists to catch, reintroduced one layer up. Both now go through the SDK; the tier flow throws rather than dropping responses; and a new offline check asserts every Retell path we write is one the vendor's SDK serves, proven to fail on both bugs before being trusted.*
*v4.5 · 2026-09-14 — Retell deprecated `/v2/list-calls` and `/list-phone-numbers`. The replacements answer a different shape rather than an error, so the drift would have been silent: `{ items }` instead of a bare array, a renamed agent filter, and no `transcript` on the list item at all — which would have emptied the full text of every backfilled call. Both list calls now go through `retell-sdk` so the version moves with the API instead of against it, `listCallsForAgent()` throws rather than answering `[]` on a rejection, `/admin/sync-calls` reports `failed`, and a new live contract check runs on every PR. Nothing on the call-answering path was touched.*
*v4.4 · 2026-09-10 — pricing reconciled with Stripe. The Starter and Pro Payment Links had been charging the original £29 and £59 while the site advertised £49 and £89, so every signup on those two tiers underpaid by 40%; new prices and links were created (a link's price cannot be edited) and `LIVE_URLS` updated. Found while fixing it: the £249 Agency product was missing from `PRODUCT_TO_PLAN` and the new Business/Agency links carried no `metadata.plan`, so `planFromStripeSession()` fell through to its `'starter'` default — **every Agency signup was provisioned as Starter**, on a 50-call allowance and the cheapest voice. Both are fixed, and Derbyshire Renewables moves from the legacy £59 to £89 at their 16 Sept renewal, unprorated. Also §8.12: `docs/ONBOARDING.md`'s "stale" £29/£59/£119 was not stale — it described what Stripe was actually charging.*
*v4.3 · 2026-09-10 — lead extraction: 17 of the first 20 leads held nothing but a phone number while the transcripts plainly named the caller and the job. Cause was not the extraction rules but the recovery path — Retell's `call_analyzed` is the event that goes missing, and `POST /admin/sync-calls` skipped every call whose row already existed and never wrote a lead at all, reporting success each run. Extraction is now one shared module with 20 unit tests (server suite 71 → 91), the backfill reconciles instead of inserting, and two live bugs in the summary fallback are fixed: the name label was matched case-sensitively in lowercase, so `"Caller: Tito"` never matched, and a single-word first name was rejected outright. `call_analyzed` also no longer blanks a stored summary when it arrives without analysis.*
*v4.2 · 2026-09-10 — the Notion Sync workflow had failed 25 runs straight, emailing `failed: 5` with no cause; the integration had simply lost access to the Subscribers and Call Log databases. `syncAll()` now probes each database once before working and reports the provider's own message, and the workflow prints it — a count is not a diagnosis. Also masked a caller's phone number that was being logged verbatim to Sentry (UK GDPR, health check C14), added the `vite:preloadError` reload real users were hitting as a blank route after every deploy (C15), and gave `normaliseHour()` the unit tests it never had after five incidents (C12). The remaining escalations are ops, not code: Derbyshire Renewables has taken zero calls in 69 days, and `NOTION_LEADS_DB_ID` is still unset.*
