...
import { CalendarAuthError, calendarConnection, calendarIsConnected, probeCalendar } from '../../services/calendar';
...
// Diagnose a tenant's diary: resolve their connection, ask the provider for
// today's busy windows, and report exactly what came back.
//
// Was ~70 lines of hand-rolled Google token refresh and freeBusy, which meant
// the diagnostic for an Outlook or Apple tenant would have reported "no
// calendar connected" for a perfectly good diary. Going through the adapter
// also means running this updates calendar_status as a side effect: a stale
// connection is flagged, and a recovered one is cleared.
...
router.get('/:id/test-calendar', async (req: Request, res: Response) => {
  ...
  const connection = calendarConnection(client as Client);
  if (!connection) {
    res.json({
      success: false,
      error: 'No diary connected — this tenant can capture a lead but cannot book a job.',
    } satisfies ApiResponse);
    return;
  }

  try {
    const { provider, calendarId, busy } = await probeCalendar(connection);
    res.json({
      success: true,
      data: {
        provider,
        calendar_id: calendarId,
        busy_windows_next_24h: busy.length,
        busy: busy.map((w) => ({ start: w.start.toISOString(), end: w.end.toISOString() })),
      },
    } satisfies ApiResponse);
  } catch (err: unknown) {
    const authFailure = err instanceof CalendarAuthError;
    res.json({
      success: false,
      error: authFailure
        ? `${connection.provider} rejected the stored credential — the tenant must reconnect. ${err.message}`
        : `Calendar probe failed: ${err instanceof Error ? err.message : String(err)}`,
    } satisfies ApiResponse);
  }
});
